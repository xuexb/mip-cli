 ...
 * @return {string} 注入后内容
 */
exports.pageInject = function (content, extensionsDir) {
    ...
    return content.replace(regex, ($0, script, src) => {
        ...
        if (m) {
            const localExtensionsFilePath = path.join(extensionsDir, m[1], m[0]);
            if (!fs.existsSync(localExtensionsFilePath)) {
                cli.log(cli.chalk.yellow('\t inject failed, not found ' + m[1]));
                return $0;
            }
